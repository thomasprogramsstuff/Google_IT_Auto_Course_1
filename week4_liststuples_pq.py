...
for filename in filenames:
    if filename.endswith(".hpp"):
        ...
        newfilenames.append('"{}"'.format(filename)) 
    else:
        newfilenames.append('"{}"'.format(filename))
...
#Code is INCORRECT; can't figure out how to not print single quote mark (')
...
#read, write, and execute for the owner, group, and others. Each of the three values can be expressed as an octal number summing each permission, with 4 corresponding to read, 2 to write, and 1 to execute. Or it can be written with a string using the letters r, w, and x or - when the permission is not granted.
 #For example: 
